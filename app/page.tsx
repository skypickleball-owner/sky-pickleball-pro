    ...
    return (
      <main className="min-h-screen bg-[#EAB308] flex items-center justify-center p-4 font-bold">
        <div className="bg-white border-8 border-black p-8 shadow-[15px_15px_0px_0px_black] text-center max-w-lg w-full">
          ...
          <p className="mb-6 uppercase text-xs">Radar Admin sudah berbunyi.<br/>Silakan lapor via WA untuk aktivasi jadwal Anda.</p>
          <div className="space-y-4">
            <button onClick={() => window.open(`https://wa.me/6289616265702?text=KONFIRMASI%20BOOKING%20SKY:%20${savedBooking.user_name}`, "_blank")} className="bg-[#25D366] text-white p-5 w-full text-xl font-black uppercase shadow-[6px_6px_0px_0px_black] active:translate-y-1 active:shadow-none transition-all">LAPOR KE ADMIN (WA)</button>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {courts.map((court) => {
                ...
                <button key={court.id} onClick={() => setSelectedCourt(court)} className={`relative h-48 border-4 border-black flex items-center justify-center overflow-hidden transition-all ${selectedCourt?.id === court.id ? "bg-[#F472B6] scale-[1.02]" : "bg-white shadow-[8px_8px_0px_0px_black] hover:-translate-y-1"}`}>
                  ...
                  {/* REVISI V4.1: BACKGROUND TAJAM, JELAS, TIDAK BLUR */}
                  <Image 
                    src={bgImg} 
                    alt="Court" 
                    fill 
                    className={`object-cover transition-opacity ${selectedCourt?.id === court.id ? 'opacity-70' : 'opacity-100 hover:opacity-80'}`} 
                  />
                  
                  {/* Overlay subtle agar teks kontras tetapi tetap terang */}
                  <div className="absolute inset-0 bg-black/5" />

                  {/* Logo Unit di depan */}
                  <div className="absolute inset-4 z-10 p-2">
                    <Image src={court.type === 'FUTSAL' ? '/logo-futsal.png' : '/logo-pb.png'} alt="Logo" fill className="object-contain drop-shadow-xl" />
                  </div>
