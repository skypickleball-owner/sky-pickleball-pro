...
export default function Home() {
  ...
  const sendToWA = () => {
    const adminWA = "6289616265702"; 
    const msg = `KONFIRMASI BOOKING SKY:%0A%0A` +
                `Nama: ${savedBooking.user_name}%0A` +
                `Unit: ${savedBooking.court_name}%0A` +
                `Jadwal: ${savedBooking.booking_date}%0A` +
                `Jam: ${savedBooking.start_time} (${savedBooking.duration} Jam)%0A` +
                `Total: Rp ${savedBooking.total_price.toLocaleString('id-ID')}%0A%0A` +
                `BUKTI BAYAR: ${savedBooking.receipt_url}`;
    window.open(`https://wa.me/${adminWA}?text=${msg}`, "_blank");
  };

  if (isSuccess) {
    return (
      <main className="min-h-screen bg-[#EAB308] flex items-center justify-center p-4 font-bold tracking-tight">
        <div className="bg-white border-8 border-black p-8 shadow-[15px_15px_0px_0px_black] text-center max-w-lg w-full">
          ...
          <h1 className="text-4xl font-black mb-4 uppercase italic tracking-tighter">MANTEP DOK!</h1>
          <div className="space-y-4">
            <button onClick={sendToWA} className="bg-[#25D366] text-white p-5 w-full text-xl font-black uppercase shadow-[6px_6px_0px_0px_black] active:translate-y-1 active:shadow-none transition-all">KIRIM KONFIRMASI KE WA</button>
    ...
    <main className="min-h-screen bg-[#EAB308] text-black font-bold pb-10 overflow-x-hidden flex flex-col">
      <header className="relative min-h-[500px] flex items-center justify-center border-b-[10px] border-black p-6">
        ...
        <div className="relative z-10 w-full max-w-6xl flex flex-col md:flex-row justify-between items-center gap-8">
          <a href="https://maps.app.goo.gl/cJZfpdYqBwp7J3EB9" target="_blank" className="bg-[#FDE047] border-4 border-black p-3 shadow-[6px_6px_0px_0px_black] hover:rotate-3 transition-transform">
            <Navigation size={32} />
            <p className="text-[10px] font-black uppercase tracking-wider text-center">CEK LOKASI</p>
          </a>

          <div className="flex flex-col items-center">
             {/* REVISI: SLOGAN #1 FAMILY SPORTAINMENT DENGAN BLOK PINK */}
          ...
          <div className="bg-[#F472B6] border-4 border-black p-2 shadow-[6px_6px_0px_0px_black] rotate-3 hover:shadow-none hover:rotate-0 transition-all">
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8 mt-12 px-4 flex-grow">
        <div className="lg:col-span-2 space-y-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {courts.map((court) => (
                <button key={court.id} onClick={() => setSelectedCourt(court)} className={`relative h-48 border-4 border-black flex items-center justify-center overflow-hidden transition-all ${selectedCourt?.id === court.id ? "bg-[#F472B6] scale-[1.02]" : "bg-white shadow-[8px_8px_0px_0px_black] hover:-translate-y-1"}`}>
                  <div className="absolute inset-4 z-10 p-2">
                    <Image src={court.type === 'FUTSAL' ? '/logo-futsal.png' : '/logo-pb.png'} alt="Logo" fill className="object-contain" />
                  </div>
                  <div className="absolute top-2 left-2 z-20">
                    <span className="bg-black text-white px-3 py-1 text-sm font-black uppercase italic shadow-[2px_2px_0px_0px_black] tracking-wider">{court.name}</span>
                  </div>
                </button>
              ))}
            </div>
          ...
          <section className="bg-white border-4 border-black p-6 shadow-[10px_10px_0px_0px_black] grid grid-cols-1 md:grid-cols-2 gap-8">
             <div className="space-y-4">
                ...
                <input type="date" onChange={(e) => setSelectedDate(e.target.value)} className="w-full p-4 border-4 border-black bg-[#FDE047] font-black outline-none tracking-tight focus:bg-white" />
                <div className="flex gap-2 font-black">
                  <select onChange={(e) => setSelectedTime(e.target.value)} className="flex-1 p-4 border-4 border-black bg-white outline-none">
             ...
             <div className="space-y-4 font-black uppercase">
                ...
                <input placeholder="NAMA LENGKAP" onChange={(e)=>setCustName(e.target.value.toUpperCase())} className="w-full p-4 border-4 border-black outline-none focus:bg-white" />
                <input placeholder="NOMOR WA" type="tel" onChange={(e)=>setCustWA(e.target.value)} className="w-full p-4 border-4 border-black outline-none focus:bg-white" />
             </div>
          </section>
        </div>
        ...
        <aside className="lg:col-span-1">
          <div className="bg-[#F472B6] border-8 border-black p-6 shadow-[12px_12px_0px_0px_black] sticky top-6">
            <h2 className="text-3xl font-black uppercase mb-2 border-b-4 border-black pb-2 italic text-shadow">Ringkasan</h2>
            {selectedCourt ? (
              <div className="space-y-6">
                <div className="bg-white border-4 border-black p-2 flex items-center justify-center gap-3 animate-pulse border-4 border-black">
                  <Hourglass className="text-red-600" size={20} />
                  <p className="text-xl font-black font-mono tracking-tighter">TIMER: {Math.floor(timeLeft/60).toString().padStart(2,'0')}:{(timeLeft%60).toString().padStart(2,'0')}</p>
                </div>
                ...
                <div className="bg-white p-4 border-4 border-black shadow-[4px_4px_0px_black]">
                   <p className="text-[10px] uppercase font-black mb-1 italic underline text-blue-600 font-black italic">TRANSFER: BANK BCA - 4585573541 a/n DINAH PATRICIA</p>
                ...
                <div className="bg-black text-white p-4 border-4 border-white transform -rotate-2 shadow-xl border-4 border-black">
                  <p className="text-[10px] uppercase mb-1 font-black">Total ({duration} Jam):</p>
                  <p className="text-4xl font-black text-center text-[#FDE047] italic">Rp {totalPrice.toLocaleString('id-ID')}</p>
                </div>
      ...
      {/* REVISI: FOOTER IDENTITAS SKY */}
      <footer className="mt-20 py-8 bg-black text-white border-t-8 border-black text-center">
         <p className="text-lg md:text-2xl font-black uppercase tracking-[0.2em] italic">SKY PICKLEBALL & FUTSAL BOOKING SYSTEM</p>
      </footer>
    </main>
  );
}